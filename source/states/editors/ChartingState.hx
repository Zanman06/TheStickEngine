...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	[
		...
		["Change Stage", "Value 1: Stage name"] //Added By Zanman06, not yet functional
	];
		...
		if(FlxG.sound.music != null)
		{
			if(PsychUIInputText.focusOn == null) //If not typing anything
			{
				...
				if(FlxG.keys.pressed.RBRACKET || (FlxG.keys.pressed.LBRACKET && (goingBack = true)))
				{
					...
					if (playbackRate > 0.45 || playbackRate < 0.55)
					{
						playbackRate = 0.5;
						setPitch();
					}
					if (playbackRate > 1.95 || playbackRate < 2.05)
					{
						playbackRate = 2;
						setPitch();
					}
	...
	function addSectionTab()
	{
		...
		objY += 25;
		changeBpmStepper = new PsychUINumericStepper(objX, objY, 1, 0, 1, 500, 3);
		...
		beatsPerSecStepper = new PsychUINumericStepper(objX + 150, objY, 1, 4, 1, 32, 2);
	...
	function addFileTab()
	{
		...
		btnY++;
		btnY += 20;
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Preview (Escape)', openEditorPlayState, btnWid);
		btn.text.alignment = LEFT;
		tab_group.add(btn);
