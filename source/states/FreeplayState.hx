...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		scoreBG = new FlxSprite(scoreText.x - 6, 0).makeGraphic(1, 155, 0xFF000000);
		scoreBG.alpha = 0.6;
		add(scoreBG);

		diffText = new FlxText(scoreText.x, scoreText.y + 125, 0, "", 24);
	...
	override function update(elapsed:Float)
	{
		...
		lerpMisses = Math.floor(FlxMath.lerp(intendedMisses, lerpMisses, Math.exp(-elapsed * 12)));

		if (Math.abs(lerpScore - intendedScore) <= 10)
			lerpScore = intendedScore;
		if (Math.abs(lerpRating - intendedRating) <= 0.01)
			lerpRating = intendedRating;
		if (Math.abs(lerpMisses - intendedMisses) <= 1)
